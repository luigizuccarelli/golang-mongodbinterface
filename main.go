...
func startHttpServer(cfg Config) *http.Server {
	...
	r.HandleFunc("/api/v1/publications/{affiliateid}", MiddlewareDBGetAllPublicationsByAffiliate).Methods("GET")
	r.HandleFunc("/api/v1/stocks/{publicationid}", MiddlewareDBGetAllStocks).Methods("GET")
	r.HandleFunc("/api/v1/stocks/{id}", MiddlewareDBUpdateStock).Methods("POST", "PUT")
